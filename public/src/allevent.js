...
import { addLikeRoute, deleteLikeRoute, getAllEventRoute, getMyIdRoute, getMyLikesRoute } from "./utils/APIRoutes"
...
const Allevent = () => {
    ...
    const handleClick = async (e, event_id) => {
        ...
        if(username){
            ...
            toast.error("Please login to like events",toastOptions)
        }
                ...
                <div className="events">
                    ...
                    {Event.map(event => (
                        <div key={event.event_id}>
                            ...
                            <EventCard onEventClick={(e) => handleClick(e, event.event_id)} className="event_card" title={event.event_name} description={event.event_desc} date={event.date} time={event.time} location={event.location} isLiked={likedEvents ? likedEvents.includes(event.event_id) : false} image={`http://127.0.0.1:8000${event.image.slice(13)}`} />
                        </div>
                    ))}
                </div>
            </Container>
